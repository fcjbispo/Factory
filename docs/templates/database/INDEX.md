...
Every schema change must be reflected here **before** it reaches the code.
...
- `changelog.md`: **append-only** log — never edit previous entries, only add new ones.
- `data-policies.md`: reviewed by @security-analyst. Classifies each field containing sensitive data or PII.
