...
This guide defines how to create, update, maintain, and navigate the documentation of this project. It is intended for both humans and AI agents.

---

## Philosophy

This structure was designed for the paradigm of **AI-assisted development**. This implies different principles from traditional documentation:

**1. Machine readability is as important as human readability.**
Documents have structured frontmatter, explicit status, and declared owners. Agents don't "browse" — they search and read. Make the path obvious.

**2. One document = one responsibility.**
Don't mix architectural decisions with feature specifications. Don't mix runbooks with postmortems. Atomic documents are easier to find, update, and replace.

**3. The INDEX.md of each section is the source of truth about what exists.**
...
No document is permanent. Every document has a status. `deprecated` or `replaced-by` documents are not deleted — they are marked and remain as history.

**5. Updates are mandatory.**
An outdated document is worse than no document: it misleads agents and humans. If something changed, update or deprecate it.
...
status: draft | under-review | active | deprecated | replaced-by: [path/to/new-file.md]
owner: senior-architect | fullstack-developer | db-architect | code-reviewer | qa-tester | devops-sre | security-analyst | po
readers: [list of agents that should read this document]
updated: YYYY-MM-DD
related:
...
- Change affecting multiple modules or teams
...
**Numbering**: sequential, zero-padded to 4 digits. Never reuse a number.

**ADR states**: `proposed` → `accepted` → `deprecated` | `replaced-by: [adr/NNNN-new.md]`
...
**What goes here**: formal API specifications (OpenAPI), GraphQL schemas, event/message contracts, webhooks.

**What does NOT go here**: internal function or class documentation (that is code documentation).
...
**Expected documents**:
- `overview.md`: high-level overview — mandatory, the first document all agents read
...
**When to create**: for any feature involving more than one agent or non-trivial design decisions. Simple features (straightforward CRUD) can be implemented directly based on acceptance criteria.
...
**What goes here**: security policies, threat models, record of treated vulnerabilities.
...
**Critical rule**: active vulnerabilities do not live in this repository — they are managed in a private channel and reported to the PO. This repository only records already treated vulnerabilities, as history.
...
**What goes here**: testing strategy, project test pyramid, coverage thresholds, test environments, test data.
...
**What goes here**: decisions made by the PO that impact the product — prioritizations, scope changes, business trade-offs, persona definitions, non-functional requirements.
...
1. Use the `_template.md` of the section
2. Fill the frontmatter completely
3. Add the entry in the `INDEX.md` of the section
4. If the document replaces another, update the old one's status to `replaced-by: [new-file.md]`

**When updating a document**:
1. Update the `updated` field in the frontmatter
...
**Review periodicity**:
- `architecture/overview.md`: review every major release
- `adr/`: never edit an accepted ADR — create a new one that replaces it
- `operations/runbook.md`: review after each incident
- `security/policies.md`: review every 6 months or after security incident
- `testing/test-strategy.md`: review when testing strategy changes
