...
> **Note on `CLAUDE.md` files**: `src/CLAUDE.md` is the project identity template copied to every project on `init` or `adopt`. The file at the repository root (`./CLAUDE.md`) is for maintaining the framework itself and its demo. They serve different purposes and should not be confused.
